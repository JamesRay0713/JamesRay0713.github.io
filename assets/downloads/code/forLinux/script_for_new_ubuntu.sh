...
# 使用方法：`bash config_script.sh [--in-container]`，然后重新打开终端，如果安装了docker，需要重启Linux。

###### 脚本大纲
# 0. 准备参数和最基础的安装包(sudo, tzdata, ca-certificates)
# 1. 配置 apt 源（默认清华源）
# 2. 安装依赖包
# 3. 安装 nvidia-docker
# 4. 安装 nvidia-docker-compose
###### TODO
# - 方便图片上传到图床，安装qshell

# 0. 准备参数和最基础的安装包
in_container=false    # 默认要执行所有模块（即非容器的场景）
apt_mirror="aliyun"  # 可以设置为 "tsinghua" 或 "aliyun"
...
EOF
  ...
  echo "Already using $apt_mirror mirrors."
fi
sudo apt update
sudo apt upgrade -y
...
check_and_install() {
  ...
  if ! command -v $package &> /dev/null; then
    ...
    # 注：psmisc用于pstree
packages=("cmake" "curl" "g++" "gcc" "git" "gnupg" "htop" "iputils-ping" "kmod" "language-pack-en"
"net-tools" "psmisc" "tmux" "tree" "vim" "wget" "zsh" )
...
if [ "$in_container" = true ]; then
  ...
  # 4. 安装 Miniconda。可根据需求, 去tuna官网中查找合适的`file`(我这里用的是py3.9版本)，或直接用最新版`Miniconda3-latest-Linux-$arch.sh`
  arch=$(uname -m)
...
EOL
  ...
  if command -v docker >/dev/null 2>&1; then
    ...
    # 设置软件库
    codename=$(grep VERSION_CODENAME /etc/os-release | cut -d'=' -f2)
    docker_official_repository="https://download.docker.com/linux/ubuntu"           # 官方docker源
    docker_aliyun_repository="https://mirrors.aliyun.com/docker-ce/linux/ubuntu"    # 阿里云的镜像，我选它，因为下载速度快
    echo \
    ...
    # 使当前用户有足够的权限连接到 Docker 守护进程, 即使用docker命令不必加`sudo`，但要重启Linux才能生效
    if [ "$(id -u)" != "0" ]; then
      sudo usermod -aG docker $USER
    else
      echo "- - - - - 未将当前用户加入docker守护进程"
    fi
    echo "- - - - - $(docker -v)"
    ...
    if [ $? -eq 0 ]; then
      echo "- - - - - Successful! execuating 'sudo docker hello world."
    fi  
  fi
      ## 后更换docker镜像源
  ...
  # 6.（可选）深度学习基础配置—— 装NVIDIA Container Toolkit: 当想在容器里跑GPU，就得给本宿主机装 Toolkit
      ...
      # TODO: 官方的repository超级慢，有没有什么镜像源？
...
if [ "$arch" == "x86_64" ]; then
  ...
  tar -xf ~/packages/vscode_cli.tar.gz
else
